...
class _LectionsWidgetState extends State<LectionsWidget>
  ...
  final _isVideoDownloaded = List.filled(999, false);
  final _textOnDownloadedPlaceHolder = List.filled(999, "");
  final _pathToSavedVideo = List.filled(999, "");
  final _fileSizeList = List.filled(999, "");
  ...
  void fetchDataPersons() async {
    ...
    final list = prefs
        .getKeys()
        .where((String key) =>
        key.toString().toLowerCase().contains("lections_downloaded"))
        .toList();
    for (int i = 0; i < list.length; i++) {
      var number = int.parse(list[i].split("lections_downloaded_")[1]);
  ...
  void DownloadFunction(int index) async {
    ...
    prefs.setBool("lections_downloaded_${index}", true);
    setState(() {
      ...
      if (File(_pathToSavedVideo[index]).existsSync()) {
        ...
        download(_postsJson[index]['path'], "lections_${_postsJson[index]['id']}.mp4",
  ...
  void DeleteFile(int index) async {
    ...
    _pathToSavedVideo[index] = "$_tempPath/${_postsJson[index]['id']}.mp4";
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.remove("lections_downloaded_${index}");
